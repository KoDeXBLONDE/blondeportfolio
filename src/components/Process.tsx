...
export function Process() {
  return (
    <section id="process" className="py-24 md:py-32 bg-slate-900/50 relative border-t border-slate-800/50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-20">
          ...
          >
            Une méthode <span className="text-brand-primary">éprouvée</span>.
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 lg:gap-4 relative z-10">
            {steps.map((step, index) => (
              ...
              >
                <div className="w-16 h-16 rounded-full bg-slate-950 border-2 border-slate-800 flex items-center justify-center text-slate-400 mb-6 group-hover:border-brand-primary group-hover:text-brand-primary transition-colors relative z-10 shadow-lg">
                  {step.icon}
                  <div className="absolute -top-3 -right-3 text-xs font-mono font-bold text-slate-400 bg-slate-800 px-2 py-1 rounded">
                    {step.number}
                  </div>
                </div>
